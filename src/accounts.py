...
「株式・金融市場・特定銘柄（ティッカー）」に関連する内容だけを抽出して要約してください。

【対象アカウントと注目点】
{accounts_block}

【厳守事項】
- 株式・市場に無関係な投稿（政治の一般論、私生活、選挙運動など）は完全に無視すること
- 該当する投稿が無いアカウントは見出しの下に「該当なし」とだけ書くこと
- 各項目は「銘柄/ティッカーまたは対象：要点（投稿の趣旨）／（投稿日 M/D）」の形式
- 推測で銘柄を補わない。投稿に明記された情報のみ
- 太字は *単一アスタリスク* で囲む（**二重** は Slack で効かないので禁止）

【出力形式】

*🏛️ @pelositracker（議員の株取引開示）*
• 銘柄/ティッカー：要点 ／（M/D）
（無ければ「該当なし」）

*🇺🇸 @realDonaldTrump（市場に影響する発言）*
• 対象：要点 ／（M/D）
（無ければ「該当なし」）
